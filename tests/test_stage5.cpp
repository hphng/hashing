#include "../stage5_double_hashing/hash_map.h"
...
void find_key_past_tombstone() {
    HashMap<std::string, int> mp(11, 0.6f);
    mp.insert("alice", 1);
    mp.insert("bob", 2);
    mp.insert("candice", 3);
    mp.remove("candice");
...
void stress_multiple_resizes() {
    HashMap<int, int> mp(11, 0.7f);
    ...
    expect(all_correct, "stress_multiple_resizes");
    expect(mp.size() == 1000, "stress_multiple_resizes_size");
}

int main() {
    std::cout << "__________________common testcase________________________" << std::endl;
    run_common_tests();
    std::cout << "__________________stage 5 only________________________" << std::endl;
    get_after_remove();
    find_key_past_tombstone();
    overwrite_key_past_tombstone();
    size_after_remove_and_reinsert();
    // stress_multiple_resizes();
    return 0;
}
